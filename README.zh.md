...
> `outline`和`box-shadow`虽然不是`border`的相关属性，但是会受到其形状影响，因此个人习惯上放在一起。
...
- [`font-kerning`](https://developer.mozilla.org/en-US/docs/Web/CSS/font-kerning)：设置字间距，注意要和`letter-spacing`区分。
